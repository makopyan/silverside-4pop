...
-   [LD](#ld)
-   [AFD](#afd)
-   [PCA on Inversion Regions](#pca-on-inversion-regions)
-   [Summarize recombination rates from brec into
    windows](#summarize-recombination-rates-from-brec-into-windows)
-   [Output bed file with region positions (centromeres, telomeres,
    inversions)](#output-bed-file-with-region-positions-centromeres-telomeres-inversions)
-   [Obtain exon densities from annotation and GC content from
    genome](#obtain-exon-densities-from-annotation-and-gc-content-from-genome)
-   [Summarize tandem repeat content from TRF and
    TRFparser](#summarize-tandem-repeat-content-from-trf-and-trfparser)
-   [Run bedtools intersect to obtain feature summaries in
    windows](#run-bedtools-intersect-to-obtain-feature-summaries-in-windows)
-   [Create dataframe with genome stats for 50kb
    windows](#create-dataframe-with-genome-stats-for-50kb-windows)
-   [Prepare input file of all population summary statistics from Angsd
    output](#prepare-input-file-of-all-population-summary-statistics-from-angsd-output)
-   [Combine genome feature and population genetic data
    frames](#combine-genome-feature-and-population-genetic-data-frames)
...
![](data_files/figure-gfm/compare-1.png)<!-- -->
...
#### Find the standard deviation of the second peak by fitting the right part of it to a normal distribution
    ...
    ##   <dbl>   <dbl> <chr>   
    ## 1   274 1713518 angsd   
    ## 2   302 1691673 samtools

    ## [1] "Optimal SD based on ANGSD: 77"

![](data_files/figure-gfm/sd-1.png)<!-- -->

#### Use two standard deviations from the mode as depth cutoffs

![](data_files/figure-gfm/fit-1.png)<!-- -->

    ## [1] "The fitted normal distribution matches the empricial depth distribution quite well."

    ## [1] "We will use its standard deviation, 77, to set up the depth filters"

    ## [1] "Max depth (mode+2sd): 428"

    ## [1] "Percent of all sites lost if max filter is used: 2.11"

    ## [1] "Percent of final mapped data will be lost if max filter is used: 4.78"

    ## [1] "Min depth (mode-2sd): 120"

    ## [1] "Percent of all sites lost if min filter is used: 23.19"

    ## [1] "Percent of final mapped data will be lost if min filter is used: 4.56"

![](data_files/figure-gfm/filters-1.png)<!-- -->

## SNP calling

BAM files: 42 populations with 4 individuals each (168 total samples)

BAM list: data\_files/bamlist42x4.txt

Reference genome: Mmenidia\_refgenome\_anchored.all\_renamed\_v2.fasta

Genome accession: GCA\_965154125.1
...
After the initial SNP calling, we created filtered SNP lists in various
formats required for downstream analyses.
...
![](data_files/figure-gfm/plot-pop-depths-1.png)<!-- -->

## Allele frequencies

We calculated allele frequencies separately for each population using
the filtered SNP list.
...
We generated folded site frequency spectra for each population
separately.
...
See script `/scripts/get_thetastats_win.sh` and
`/scripts/get_theta_per_pop.sh`
...
``` r
trf_output <- read_tsv("trf/Mmenidia_refgenome_anchored.all_renamed_v2.fasta.2.7.7.80.10.50.500.final.parse")

numbs <- read_csv("trf/charvect.csv") %>% rename("chr"=x)
